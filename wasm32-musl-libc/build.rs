...
fn build_libc() {
    ...
    if lib_path.exists() {
        ...
        println!(
            "cargo:warning=Copied wasm32_musl_libc.a to {}",
    ...
    if cfg!(feature = "link-libc") {
        ...
        if target == "wasm32-unknown-unknown" {
            ...
            if lib_path.exists() {
                ...
                println!("cargo:rustc-link-lib=static=wasm32_musl_libc");
                println!("cargo:warning=Linking with pre-compiled libwasm32_musl_libc.a");
            } else {
                println!(
                    "cargo:warning=Pre-compiled libwasm32_musl_libc.a not found in lib/ directory"
                );
                println!(
