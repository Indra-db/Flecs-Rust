...
fn try_add_relationship<T: ComponentId>(world: &WorldGuard)
where
    (flecs_ecs::core::utility::id::Id<T>, flecs_ecs::core::Entity): InternalIntoEntity,
    (flecs_ecs::core::utility::id::Id<T>, u64): flecs_ecs::core::InternalIntoEntity,
{
    let obj = world.entity();
    let e = world.entity().add((T::id(), obj.id()));
    assert!(e.has((T::id(), flecs::Wildcard::ID)));
    e.remove((T::id(), flecs::Wildcard::ID));
    assert!(!e.has((T::id(), flecs::Wildcard::ID)));
}

fn try_add_second<T: ComponentId>(world: &WorldGuard)
where
    (flecs_ecs::core::Entity, flecs_ecs::core::utility::id::Id<T>): InternalIntoEntity,
    (u64, flecs_ecs::core::utility::id::Id<T>): flecs_ecs::core::InternalIntoEntity,
{
    let obj = world.entity();
    let c = world.component::<T>();
    let e = world.entity().add((obj.id(), c));
    assert!(e.has((flecs::Wildcard::ID, T::id())));
    e.remove((flecs::Wildcard::ID, T::id()));
    assert!(!e.has((flecs::Wildcard::ID, T::id())));
}
...
fn no_default_ctor_add_relationship() {
    ...
    world.component::<&NoDefault>();
    try_add_relationship::<NoDefault>(&world);
}
...
fn no_default_ctor_add_second() {
    ...
    world.component::<&NoDefault>();
    try_add_second::<NoDefault>(&world);
}
...
fn on_replace_hook() {
    ...
    assert_eq!(world.cloned::<&Count>().0, 3);
    let v = e2.cloned::<&Position>();
    assert_eq!(v.x, 5);
    assert_eq!(v.y, 6);
}
