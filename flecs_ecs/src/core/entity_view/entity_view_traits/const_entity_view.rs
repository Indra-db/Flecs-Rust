...
pub trait ConstEntityView<'w>: WorldProvider<'w> + EntityId + Sized {
    ...
    fn try_get<T: GetTuple>(&self, callback: impl for<'e> FnOnce(T::TupleType<'e>)) -> bool {
        ...
        let tuple_data = T::create_ptrs::<false>(self.world(), self.entity_id(), record);
    ...
    fn get<T: GetTuple>(&self, callback: impl for<'e> FnOnce(T::TupleType<'e>)) {
        ...
        let tuple_data = T::create_ptrs::<true>(self.world(), self.entity_id(), record);
        let tuple = tuple_data.get_tuple();
        world.defer_begin();
        callback(tuple);
        world.defer_end();
