...
use super::{
    ...
    IntoWorld, WorldRef, WorldT, ECS_WILDCARD,
};
    ...
    T: Iterable + 'static,
{
    ...
    fn build(&mut self) -> Self::BuiltType {
        Filter::<T>::new_from_desc(self.world.world_ref(), &mut self.desc as *mut _)
    }
}
...
pub trait FilterBuilderImpl<'a>: TermBuilder<'a> {
    ...
    fn term_with_id(&mut self, id: impl IntoEntityIdExt) -> &mut Self {
        self.term();
        unsafe {
            *self.term_ptr_mut() = Term::new_id(Option::<WorldRef>::None, id).move_raw_term();
        }
        self
    }
    ...
    fn term_with<T: InOutType>(&mut self) -> &mut Self {
        if <T::Type as IntoComponentId>::IS_PAIR {
            self.term_with_id(<T::Type as IntoComponentId>::get_id(self.world_ref()));
        } else {
            self.term();

            unsafe {
                *self.term_ptr_mut() = Term::new_id(
                    Option::<WorldRef>::None,
                    <T::Type as IntoComponentId>::get_id(self.world_ref()),
                )
                .move_raw_term();
    ...
    fn term_with_pair_id_name(
        ...
        self.term();
        unsafe {
            *self.term_ptr_mut() = Term::new_id(Option::<WorldRef>::None, first.get_id())
                .select_second_name(second)
                .move_raw_term();
        }
        self
    ...
    fn term_with_pair_first<First: ComponentId>(&mut self, second: impl IntoEntityId) -> &mut Self {
        self.term_with_id((First::get_id(self.world_ref()), second))
    }
    ...
    fn term_with_pair_name<First: ComponentId>(&mut self, second: &'static CStr) -> &mut Self {
        self.term_with_id(First::get_id(self.world_ref()))
            .select_second_name(second)
    }
    ...
    fn term_with_enum<T: ComponentId + ComponentType<Enum> + CachedEnumData>(
        ...
        let enum_id = T::get_id(self.world_ref());
        // SAFETY: we know that the enum_value is a valid because of the T::get_id call
        let enum_field_id = value.get_id_variant(self.world_ref());
